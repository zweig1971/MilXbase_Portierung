...
{ Ifk_ID_ModBus     = $FD;} {IFK 450.012 fÅr Modulbus: lesen mit Fct CC[H] }
 ...
 Fair_Fct_Reset      = $0800;   {Fair-Bus Reset fÅr IFC, gilt auch fÅr APL-IO-Bus}
...
{ asm
 ...
 begin
  KeyEPressed:=KeyPressed;
{   myResult:=GetAsyncKeyState(VK_Return) or GetAsyncKeyState(VK_SPACE);
   if myResult <> 0 then KeyEPressed:=true
   else  KeyEPressed:=false;  }
 end;

function ReadEKey: char;  //ohne keypress
{ asm
  mov ah,10h
  int 16h  }
 begin
   if ScanCode <> 0 then             //Gebe dem Anwender den Scancode
    begin
      ReadEKey := Char(ScanCode);
      ScanCode := 0;
      exit;
    end else ReadEKey := ReadChar;

 end;

function NewReadKey: Char;  //mit  keypress
 ...
 begin
   if ScanCode <> 0 then             //Gebe dem Anwender den Scancode
    begin
      ...
      ScanCode := 0;
      exit;
    end else NewReadKey := ReadKey;         //Einschlie·lich Make-Code E0 der Pfeiltasten
